...
select monthname(TIMEID),round(count(TRANSACTION_VALUE)) from cdw_sapp_credit_card group by monthname(TIMEID) order by round(sum(TRANSACTION_VALUE)) desc limit 3;
