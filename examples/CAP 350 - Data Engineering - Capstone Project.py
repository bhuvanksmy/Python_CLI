...
df_customer = spark.read.option("multiline", "true").json(
    "C:/Users/ASB/Desktop/Bhuvana/Perscholas/Capstone_Project/cdw_sapp_customer.json")
...
df_branch = spark.read.option("multiline", "true").json(
    "C:/Users/ASB/Desktop/Bhuvana/Perscholas/Capstone_Project/cdw_sapp_branch.json")
...
df_branch = df_branch.select("BRANCH_CODE", "BRANCH_NAME", "BRANCH_STREET", "BRANCH_CITY", "BRANCH_STATE",
                             expr("CASE WHEN BRANCH_ZIP IS NULL  THEN '99999' ELSE BRANCH_ZIP END").alias("BRANCH_ZIP"),
                             expr(
...
df_creditcard = spark.read.option("multiline", "true").json(
    "C:/Users/ASB/Desktop/Bhuvana/Perscholas/Capstone_Project/cdw_sapp_credit.json")
# Manipulating Credit card data according to the specifications in mapping document.
df_creditcard = df_creditcard.select("CREDIT_CARD_NO", expr("YEAR || DAY || MONTH").alias("TIMEID"), "CUST_SSN",
